...
LRESULT CALLBACK WinChildProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
   switch (message) {
...
int WINAPI WinChildMain(HINSTANCE hChildInstance,
   HINSTANCE hPrevInstance,
   LPSTR lpszArgument,
   int nFunsterStil) {

   HWND hwndC; // O objeto para indicar a janela
   MSG msg; // Objeto mensagem do Windows para controlar o acionamento de botões
   WNDCLASSEX wc; // O objeto para criar a janela

   wc.hInstance = hChildInstance;
   wc.lpszClassName = "WindowsChildAppl"; // Nome da classe da janela
   wc.lpfnWndProc = WinChildProcedure;
   wc.style = CS_DBLCLKS;
   ...
   wc.cbClsExtra = 0;
   wc.cbWndExtra = 0;
   wc.hbrBackground = (HBRUSH) COLOR_BACKGROUND + 1; // Cor cinza de background

   if (!RegisterClassEx( & wc)) // Se o registro da classe foi mal-sucedido return 0
      return 0;

   hwndC = CreateWindowEx( // Criação da janela
      0,
      "WindowsChildAppl", // Indicação da classe da janela
      NULL, // Nome da janela
   ...
   ShowWindow(hwndC, nFunsterStil); // Mostrar a janela
}
